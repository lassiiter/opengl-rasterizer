...
namespace nelems
{
    ...
    void IBL::load_background_texture(const std::string& filePath)
    {
        // from https://learnopengl.com/PBR/IBL/Diffuse-irradiance
        mBackgroundShader->use();
        mBackgroundShader->set_i1(6,"environmentMap");
        ...
        if (data)
        {
            ...
            std::cout << "Successfully loaded: " << filePath.c_str() << std::endl;
        ...
        mEquirectangularToCubemapShader->set_i1(6, "equirectangularMap");
        ...
        mIrradianceShader->set_i1(6, "environmentMap");
    ...
    {
            if (mRenderBufferMgr->is_intialized() == 0)
            {
                ...
                mRenderBufferMgr->create_buffers(vertices);
            }
            // render Cube
            glDepthFunc(GL_LEQUAL);
            mRenderBufferMgr->bind();
            mRenderBufferMgr->draw(36); //hard coded for cube
    }
}